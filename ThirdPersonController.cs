...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [SerializeField] private float staminaConsumtion = 3f;
        ...
        public bool _isWeaponDrawed = false;
        public bool _isCastinSpell = false;
        ...
        private void Move()
        {
            ...
            if(_input.sprint && _isInWater)
            {
                _targetSpeed = MoveSpeed;
            }
            else if(_input.sprint && !_isInWater )
            {
                if(Player.MyInstance.stamina.MyCurrentValue > staminaConsumtion)
                {
                    _targetSpeed = SprintSpeed;
                    Player.MyInstance.stamina.MyCurrentValue -= staminaConsumtion * Time.deltaTime;
                }
                else
                {
                    _targetSpeed = MoveSpeed;
            ...
            Vector3 targetDirection = Quaternion.Euler(0.0f, _targetRotation, 0.0f) * inputDirection.normalized;
            ...
            if (_hasAnimator)
            {
                ...
                if (_input.sprint && !_isInWater)
                {
                    ...
                    _animator.SetFloat("X", Mathf.Lerp(x, _input.move.x * 2, _animSpeed));
                }
                else
                {   
                    float x = _animator.GetFloat("X");
                    _animator.SetFloat("X", Mathf.Lerp(x, _input.move.x, _animSpeed));
                }
               
                if (_input.sprint && !_isInWater)
                {
                    float y = _animator.GetFloat("Y");
                    _animator.SetFloat("Y", Mathf.Lerp(y, _input.move.y * 2, _animSpeed));
                }
                else
                {
                    float y = _animator.GetFloat("Y");
                    _animator.SetFloat("Y", Mathf.Lerp(y, _input.move.y, _animSpeed));
                }
            }
        }
